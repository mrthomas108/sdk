...
void ConsoleModel::autoComplete(bool forwards, unsigned consoleWidth)
{
    if (autocompleteSyntax)
    {
        if (!autocompleteState.active)
        {
            ...
            autocompleteState = autocomplete::autoComplete(u8line, insertPos, autocompleteSyntax, unixCompletions);
        ...
        insertPos = clamp<size_t>(autocompleteState.wordPos.second, 0, buffer.size());
        redrawInputLineNeeded = true;
    }
}
    ...
    if (ir.EventType == 1 && ir.Event.KeyEvent.bKeyDown)
    {
        ...
        default:
            ...
            default:
                ...
                {
                    OutputDebugStringA(std::to_string((int)ir.Event.KeyEvent.uChar.UnicodeChar).c_str());
                    OutputDebugStringA(std::to_string((int)ir.Event.KeyEvent.wVirtualKeyCode).c_str());
                }
#endif
                break;
            }
